...
def generate_launch_description():
  ...
  rviz_config_dir = os.path.join(
    get_package_share_directory('cap_tf2'), 'rviz', 'listener_point_transform.rviz')
  ...
  listener_node = Node(name='listener',
    package='cap_tf2', 
  ...
  # Simulamos el sensor que publica puntos
  point_pub_node = Node(name='point_pub',
    package='cap_tf2', 
    executable='publisher_point.py',
    )
  
  listener_point_transform_node = Node(name='listener_point_transform',
    package='cap_tf2', 
    executable='listener_point_transform.py',
    parameters = [
      {'turtlename': 'turtle1'}],
    output='screen',
    prefix='xterm -e', 
  ...
  ld.add_action(point_pub_node)
  ld.add_action(listener_point_transform_node)
