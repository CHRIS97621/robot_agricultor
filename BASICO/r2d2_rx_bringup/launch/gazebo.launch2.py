...
def generate_launch_description():
  ...
  # MAPVIZ
  #---------------------------------------------------------------------------#
  mapviz_config_dir = PathJoinSubstitution([FindPackageShare(pkg), 'config', 'mapviz_config.yaml'])

  mapviz = Node(
    package='mapviz',
    executable='mapviz',
    arguments=['-d', mapviz_config_dir],
    parameters=[{'use_sim_time': use_sim_time}],
    output='screen',
    remappings=[
        ('/odom', '/'+ prefix +'odom'),  # Remap odometry data
        ('/fix', '/'+ prefix +'navsat'),  # Remap GPS data
        ('/imu', '/'+ prefix +'imu')     # Remap IMU data
    ]
 )
