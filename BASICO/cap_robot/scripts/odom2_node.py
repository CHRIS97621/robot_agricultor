...
import math
import serial
from tf_transformations import quaternion_from_euler  # Importa la función para trabajar con cuaterniones
...
class OdomNode(Node):
    def __init__(self):
        ...
        timer_period = 0.1  # Frecuencia de actualización
        self.timer = self.create_timer(timer_period, self.update_odometry)

    def update_odometry(self):
        # Leer datos de velocidad desde el ESP32
        if self.ser.in_waiting > 0:
            line = self.ser.readline().decode('ascii').strip()
            try:
                vx, vtheta = map(float, line.split(','))   #vx,vtheta: velocidad linear y angular del robot 
            except ValueError:
                self.get_logger().warn("Datos inválidos recibidos del ESP32.")
                return
        else:
            # Si no hay datos disponibles, usa velocidades por defecto (detenerse)
            vx, vtheta = 0.0, 0.0
