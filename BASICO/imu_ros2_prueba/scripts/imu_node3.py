...
# coding:UTF-8
# Version: V2.0.1
import serial                   # Comunicaion por puerto serial#
import rclpy                    # Paquete de python para ROS2#
from rclpy.node import Node     # Importamos la clase nodo#

# Almacenmiento de la Data cruda
# LIsta de 8 elementos iguales a 0.0
ACCData = [0.0] * 8             # 8 bytes que representan la lectura en los ejes x,y,z del acelerometro#
GYROData = [0.0] * 8            # 8 bytes que representan la lectura en los ejes x,y,z del giroscopio#
AngleData = [0.0] * 8           # 8 bytes que representan la lectura en los angulos de ejes#
...
a = [0.0] * 3                   # Lecturas de acelerometro procesado x,y,z#
w = [0.0] * 3                   # Lecturas de giroscopio procesado x,y,z#
Angle = [0.0] * 3
acc = None    
gyro = None
angle = None
...
class IMUNode(Node):
    ...
    def get_gyro(self, datahex):
        ...
        wzh = datahex[5]
        k_gyro = 2000.0
        gyro_x = (wxh << 8 | wxl) / 32768.0 * k_gyro  # 15 bits
